 ...
 *  ---------------------------------------------------------------------------
 *  Copyright 2014-2023 by Alain Menu   <alain.menu@ac-creteil.fr>
 *
...
//	return QString("QamModbusMap version %1.%2").arg(MODBUSMAP_VERSION_MAJOR).arg(MODBUSMAP_VERSION_MINOR) ;
    return QString("QamModbusMap version 2.11");
}
...
*/
	...
	/*if ( request.count() < 7 )*/ if ( request.size() < 7 ) {
    ...
    /*if ( request.count() > ( 6 + mbapLen ) )*/ if ( request.size() > ( 6 + mbapLen ) ) {
    ...
    /*if ( request.count() < ( 6 + m_mbapLen ) )*/ if ( request.size() < ( 6 + m_mbapLen ) ) {
...
*/
	if ( m_verbose ) {
		QString s ;
		for ( int i = 7 ; i < /*request.count()*/ request.size() ; ++i ) {
	...
	case 16 :	table = ( m_funct == 15 ? Coil : HoldingRegister ) ;
				...
				/*if ( request.count() < 13 )*/ if ( request.size() < 13 ) {
				...
				/*if ( request.count() != ( 13 + n ) )*/ if ( request.size() != ( 13 + n ) ) {
					m_exception = 0x03 ;	// illegal data value
	...
	m_mbapLen = /*resp.count()*/ resp.size() - 6 ;
	resp[4] = ( m_mbapLen >> 8 ) & 0xFF ;
	resp[5] =   m_mbapLen & 0xFF ;

	if ( m_verbose ) {
		QString s ;
		for ( int i = 7 ; i < /*resp.count()*/ resp.size() ; ++i ) {
...
void QamModbusMap::responseFromServer(QByteArray response )
{
	if ( m_verbose ) {
		QString s ;
		for ( int i = 7 ; i < /*response.count()*/ response.size() ; ++i ) {
 ...
 * \param table : table concernée
 * \param name : nom de la donnée dans la table.
 * \return référence sur l'objet QamModbusData concerné, ou sur un objet de nom
 * "NULL" en cas d'échec.
 */
...
/*
    //
    qDebug() << "setLocalValue()" << m_addr << m_number << m_value << m_name;
    for ( int i = 0 ; i < m_number ; ++i ) {
        qDebug() << Qt::hex << (quint8)m_data[ i * 2 ] << (quint8)m_data[ i * 2 + 1 ] ;
    }
    //
*/
    switch ( table ) {
...
/*
    //
    qDebug() << "setRemoteValue()" << m_addr << m_number << m_value << m_name;
    for ( int i = 0 ; i < m_number ; ++i ) {
        qDebug() << Qt::hex << (quint8)m_data[ i * 2 ] << (quint8)m_data[ i * 2 + 1 ] ;
    }
    //
*/
	...
	if ( m_item ) {
		...
		m_mutex.unlock() ;
        // ... et récupère la nouvelle valeur de la donnée primaire
...
void QamModbusMap::buildAndSendWriteFrame()
{
	...
	if ( m_verbose ) {
		QString s ;
		for ( int i = 7 ; i < /*frame.count()*/ frame.size() ; ++i ) {
